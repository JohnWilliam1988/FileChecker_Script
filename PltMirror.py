...
def check_clockwise_triplets(points):
    ...
    if max_clockwise_index >= max_counter_clockwise_index:   
       for i, pt in enumerate(x_coordinates):
            if (pt < mid_x):
                mirredX = pt + 2 * (mid_x - pt)
            elif (pt > mid_x):
                mirredX = pt - 2 * (pt - mid_x)
            else:
                mirredX = mid_x
            
            if i == 0 :
                pt = 'U%d,%d ' % (mirredX, y_coordinates[i])
            else :
                pt = 'D%d,%d ' % (mirredX, y_coordinates[i])
            
            allMirroredCoordinates.append(pt)
    else:
        for i, pt in enumerate(x_coordinates):
            if i == 0 :
